...
namespace Inv.BLL.Services.GUSERS
{
    public class G_USERSService : IG_USERSService
    {
        ...
        #endregion
        #region User Branch
        public G_USER_COMPANY InsertCompUser(G_USER_COMPANY USER)
        {
            var pay = unitOfWork.Repository<G_USER_COMPANY>().Insert(USER);
            unitOfWork.Save();
            return pay;
        }
        public G_USER_BRANCH InsertBranchUser(G_USER_BRANCH USER)
        {
            var pay = unitOfWork.Repository<G_USER_BRANCH>().Insert(USER);
            unitOfWork.Save();
            return pay;
        }

        public G_USER_BRANCH UpdateBranchUser(G_USER_BRANCH USER)
        {
            var pay = unitOfWork.Repository<G_USER_BRANCH>().Update(USER);
            unitOfWork.Save();
            return pay;
        }

        public void DeleteBranchUsers(int BraCode, string UserCodeE)
        {
            var Branch = unitOfWork.Repository<G_USER_BRANCH>().Get(x => x.BRA_CODE == BraCode && x.USER_CODE == UserCodeE);
            unitOfWork.Repository<G_USER_BRANCH>().Delete(Branch);
