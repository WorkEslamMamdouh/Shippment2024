...
namespace Inv.API.Controllers
{
...

    {
        ...
        [HttpGet, AllowAnonymous]
        public IHttpActionResult GetVatHeader_Detail(string UserCode, string Token, int COMP_CODE, int VAT_YEAR, int PERIOD_CODE)
        {
            if (ModelState.IsValid && UserControl.CheckUser(Token, UserCode))
            {
                var res = new VatPeriodMatserDetail();
                var res1 = db.AVAT_PERIOD.Where(x => x.COMP_CODE == COMP_CODE && x.VAT_YEAR == VAT_YEAR && x.PERIOD_CODE == PERIOD_CODE).FirstOrDefault();
                var res2 = db.AQVAT_GetPeriodDetail.Where(x => x.COMP_CODE == COMP_CODE && x.VAT_YEAR == VAT_YEAR && x.PERIOD_CODE == PERIOD_CODE).ToList();
                var salesVat = res2.Where(x => x.TYPE == 1).ToList();
                decimal? vatsales=0;
                decimal? Updsales = 0;
                for (int i = 0; i < salesVat.Count; i++)
                {
                    vatsales += salesVat[i].Val_Amount;
                    Updsales += salesVat[i].Upd_Amount;
                }
                var PurVat = res2.Where(x => x.TYPE == 2).ToList();
                decimal? vatPur=0;
                decimal? UpdPur = 0;
                for (int i = 0; i < PurVat.Count; i++)
                {
                    vatPur += PurVat[i].Val_Amount;
                    UpdPur += PurVat[i].Upd_Amount;
                }
                res.vatsales = vatsales;
                 res.Updsales = Updsales;
                 res.vatPur = vatPur;
                 res.UpdPur = UpdPur;
                res.AVAT_PERIOD = res1;
                res.AQVAT_GetPeriodDetailSales= salesVat;
                res.AQVAT_GetPeriodDetailPur= PurVat;
                return Ok(new BaseResponse(res));
            }
            return BadRequest(ModelState);
        }
