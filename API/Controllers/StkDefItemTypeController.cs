...
namespace Inv.API.Controllers
{
    public class StkDefItemTypeController : BaseController
    {
        ...
        public IHttpActionResult GetItemByFamilyIdOrdered(int CompCode, int BranchCode , int FinYear, int familyid, int storeid, string UserCode, string Token)
        {
            if (ModelState.IsValid && UserControl.CheckUser(Token, UserCode))
            {
                string SQL = "Select ItemID,  Itm_DescA, Itm_DescE, UnitPrice, OnhandQty, MinUnitPrice,UomID from   I_Item  where OnhandQty>0 and ItemFamilyID= " + familyid + " and StoreId = " + storeid + " and   CompCode = " + CompCode + " and BraCode = "+ BranchCode + " order by ItemCode";
                var GetItemStore = db.Database.SqlQuery<I_Item>(SQL).ToList();
        ...
        [HttpGet, AllowAnonymous]
        public IHttpActionResult GetI_ItemByFamilyIdOrdered(int CompCode, int FinYear, int familyid,  string UserCode, string Token)
        {
            if (ModelState.IsValid && UserControl.CheckUser(Token, UserCode))
            {
                string SQL = "Select * from   I_Item  where   ItemFamilyID= " + familyid + " and CompCode = " + CompCode + " order by ItemCode";
                var GetItemStore = db.Database.SqlQuery<I_Item>(SQL).ToList();
        ...
        //**asmaa 14-12
        [HttpGet, AllowAnonymous]
        public IHttpActionResult GetAllItembyItemId(int CompCode, int BranchCode, string UserCode, string Token, int itemid)
        {
            if (ModelState.IsValid && UserControl.CheckUser(Token, UserCode))
            {
                var res = db.IQ_GetItemStoreInfo.Where(x => x.CompCode == CompCode && x.BraCode == BranchCode && x.ItemID == itemid).FirstOrDefault();
                return Ok(new BaseResponse(res));
            }
            return BadRequest(ModelState);
        }
        [HttpGet, AllowAnonymous]
        public IHttpActionResult GetAllItem(int CompCode, int BranchCode, string UserCode, string Token)
        {
            if (ModelState.IsValid && UserControl.CheckUser(Token, UserCode))
            {
                var res = db.IQ_GetItemStoreInfo.Where(x => x.CompCode == CompCode && x.BraCode == BranchCode).ToList();
                return Ok(new BaseResponse(res));
            }
            return BadRequest(ModelState);
        }
