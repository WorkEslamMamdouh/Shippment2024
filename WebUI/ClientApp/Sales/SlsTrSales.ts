...
namespace SlsTrSales {
    //system varables
    var SysSession: SystemSession = GetSystemSession(Modules.SlsTrSales);
    //// 
    var compcode: Number;
    ...
    var custom1: Array<A_Rec_D_Customer> = new Array<A_Rec_D_Customer>();
    ...
    var ItemDetails: Array<IQ_GetItemStoreInfo_New> = new Array<IQ_GetItemStoreInfo_New>();
    ...
    var txt_Tax_AfterTotalVAT: HTMLInputElement;
    var txt_Tax_Vat: HTMLInputElement;
    ...
    var btnminus_priceWithVat: HTMLButtonElement;
    var btnplus_priceWithVat: HTMLButtonElement;

    var All_item: HTMLButtonElement;
    var Sava_Basket: HTMLButtonElement;
    var btnOffer_PriceSearch: HTMLButtonElement;
    var Category_NAME;
    var CatID;
    var Num_Item;
    var x;
    ...
    var txtUnitpriceWithVat: HTMLInputElement;
    var txtTotal_Price: HTMLInputElement;
    var txtTotAfterTax_Popu: HTMLInputElement;
    var txtTax_Rate_Popu: HTMLInputElement;
    ...
    var List_IQ_GetSlsInvoiceItem: Array<IQ_GetSlsInvoiceItem> = new Array<IQ_GetSlsInvoiceItem>();
    ...
    function InitalizeControls() {
        ...
        txt_Tax_Vat = document.getElementById("txt_Tax_Vat") as HTMLInputElement;
        txt_Tax_AfterTotalVAT = document.getElementById("txt_Tax_AfterTotalVAT") as HTMLInputElement;
        ...
        Basket = document.getElementById("Basket") as HTMLButtonElement;
        btn_cancel_Popu = document.getElementById("btn_cancel_Popu") as HTMLButtonElement;
        Sava_Basket = document.getElementById("Finsh_Order") as HTMLButtonElement;
        btnOffer_PriceSearch = document.getElementById("btnOffer_PriceSearch") as HTMLButtonElement;
        ...
        btnminus_priceWithVat = document.getElementById("btnminus_priceWithVat") as HTMLButtonElement;
        btnplus_priceWithVat = document.getElementById("btnplus_priceWithVat") as HTMLButtonElement;
        ...
        txtUnitpriceWithVat = document.getElementById('txtUnitpriceWithVat') as HTMLInputElement;
        ...
        txtTax_Rate_Popu = document.getElementById('txtTax_Rate_Popu') as HTMLInputElement;
    ...
    function ddlCustomer_onchange() {
        if (ddlCustomer.value != "null") {
            ...
            custom1 = VendorDetails.filter(s => s.CustomerId == customerID);
    ...
    function BuildControls(cnt: number) {
        ...
        $(dropddlItem).change(function () {
            ...
            if ($('#ddlItem' + cnt).val() == "null") {
                ...
                if (res == true) {
                    ...
                    let GetUnitprice: IGetunitprice = Get_PriceWithVAT(itemPrice, VatPrc, flag_PriceWithVAT);
        ...
        $("#txtPrice" + cnt).on('change', function () {
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, Number($("#txtTax_Rate" + cnt).val()), false);
        ...
        $("#txtUnitpriceWithVat" + cnt).on('change', function () {
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, Number($("#txtTax_Rate" + cnt).val()), true);
    ...
    function clear() {
        ...
        SysSession.CurrentEnvironment.UserType == 2 || SysSession.CurrentEnvironment.UserType == 3 ? $('#ddlCashBox').prop('selectedIndex', 1) : $('#ddlCashBox').prop('selectedIndex', 0); $('#ddlCashBox').attr('disabled', 'disabled');
    ...
    function btnOffer_PriceSearch_onclick() {
        ...
        sys.FindKey(Modules.SlsTrSales, "btnOffer_PriceSearch", "TrType = 2 and BranchCode = " + BranchCode + " and CompCode = " + compcode + "and SlsInvSrc = 1 and Status = 1", () => {
    ...
    function Get_InvoiceItems(InvoiceID: number) {

        List_IQ_GetSlsInvoiceItem = new Array<IQ_GetSlsInvoiceItem>();
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    List_IQ_GetSlsInvoiceItem = result.Response as Array<IQ_GetSlsInvoiceItem>;
    ...
    function Display_OfferPrice() {
        ...
        for (var i = 0; i < List_IQ_GetSlsInvoiceItem.length; i++) {
            //
            let total = (List_IQ_GetSlsInvoiceItem[i].SoldQty * List_IQ_GetSlsInvoiceItem[i].Unitprice);
            CountGrid = i;
            BuildControls(CountGrid);
            ...
            var item = ItemDetails.filter(x => x.ItemFamilyID == Number(selectedFamily) && x.StoreId == StoreId);
            $('#ddlItem' + i).empty();
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(itemPrice, Number(List_IQ_GetSlsInvoiceItem[i].VatPrc), false);
        ...
        if (Men_Sales_1.getAttribute('style') == 'display: none;') {
            ...
            for (var i = 1; i < Num_Add_List + 1; i++) {
                ...
                if (prgraph != null) {
                    ...
                    let Tax_Rate = Number(prgraph.getAttribute("data_Tax_Rate"));
                    let total = (Qty * Price_Item);


                    Tax_Total += Number(Qty) * Number(Price_Item);
                    Tax_VatAmount += Number(total) * Tax_Rate / 100;
                    Tax_NetAfterVat += Total_Price;
    ...
    function btnChanege_onclick() {
        if (Men_Sales_1.getAttribute('style') == 'display: none;') {
            ...
            if (CountGrid > 0) { Display_Page_2(); } else {
                try {
                    Remove_Item_in_Basket();
                } catch (e) {
    ...
    function GetAllItem() {
        Ajax.Callsync({
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    ItemDetails = result.Response as Array<IQ_GetItemStoreInfo_New>;
                }
            }
        });
    }
    ...
    function Open_Menu() {
        ...
        if (Selecteditem.length == 0) { this.setAttribute('value', 'Finish'); this.setAttribute('Data_Finish', 'Finish'); this.setAttribute('disabled', 'disabled'); div_menu.setAttribute('style', 'display:none;'); }
        else {
    ...
    function click_Menu() {
        ...
        let GetUnitprice: IGetunitprice = Get_PriceWithVAT(itemPrice, Tax_Rate, flag_PriceWithVAT);
    ...
    function btnminus_price_onclick() {
        ...
        let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, Number($("#txtTax_Rate_Popu").val()), false);
        $("#txtUnitpriceWithVat").val(GetUnitprice.unitpricewithvat);
    }
    function btnminus_priceWithVat_onclick() {
        ...
        let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, Number($("#txtTax_Rate_Popu").val()), true);
        $("#txtPrice").val(GetUnitprice.unitprice);
        Total();
    ...
    function Add_ROW_IN_Basket() {
        ...
        if (hide == boll) { x.innerHTML = '<i id="remo" class="fa" style="margin-top: 0px;font-size: 21px;">' + num_item + '</i>'; }
        else { x.innerHTML = '<i id="remo" class="fa" style="margin-top: 0px;font-size: 21px;">' + num_item + '</i>'; }
    ...
    function txtCommission_onchange_Basket() {
        ...
        if (txtCommission_Basket.value.trim() == '') {
            ...
            let txtNet = (Number(Net) - Number(comm)).RoundToSt(2).toString();
            $('#All_Total_Basket').html('( ' + txtNet + ' )');
            $('#All_Total_Basket').attr('All_Total', txtNet);
    ...
    //------------------------------------------------------Edit-----------------------------------
    function click_Edit(New_Name: string, New_Pirce: number, new_Qet: number, tax_rate: number, New_OnhandQty: number, New_MinUnitPrice: number) {
        ////  
        ...
        for (var i = 0; i < Num_Add_List + 1; i++) {
            ...
            if (prgraph != null) {
                let Name_Item = prgraph.getAttribute("data_name_p");
                let Qty = Number(prgraph.getAttribute("data_qet_p"));
                let Total = Number(prgraph.getAttribute("data_Total_QET"));

                if (Name_Item == Name_Product) {
                    New_OnhandQty = Total - Qty;
                }
            }
        }
        ...
        let GetUnitprice: IGetunitprice = Get_PriceWithVAT(New_Pirce, tax_rate, false);
    ...
    function Display_Page_1() {
        ...
        for (var i = 0; i < List.length; i++) {
            ...
            $("#ddlItem" + i).removeAttr('disabled');
            var selectedFamily = $("#ddlFamily" + i).val();
            var item = ItemDetails.filter(x => x.ItemFamilyID == Number(selectedFamily) && x.StoreId == Number(StoreId));
            $('#ddlItem' + i).empty();
            $('#ddlItem' + i).append('<option value="' + null + '">' + (lang == "ar" ? "أختر الصنف" : "Choose item") + '</option>');
            for (var u = 0; u < item.length; u++) {
                $('#ddlItem' + i).append('<option data-UnitPrice="' + item[u].UnitPrice + '"  data-MinUnitPrice="' + item[u].MinUnitPrice + '" data-UomID="' + item[u].UomID + '" data-OnhandQty="' + item[u].OnhandQty + '" value="' + item[u].ItemID + '">' + (lang == "ar" ? item[u].Itm_DescA : item[u].Itm_DescE) + '</option>');
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(itemPrice, Number(List[i].VatPrc), false);
    ...
    function Assign_Get_Data() {
        ...
        for (var i = 1; i < Num_Add_List + 1; i++) {
            ...
            if (prgraph != null) {
                ...
                let dat_UomID = prgraph.getAttribute("data-UomID");
                ...
                let GetUnitprice: IGetunitprice = Get_PriceWithVAT(Number(Price_on_seller), Number(Tax_Rate), false);
                Model.Unitprice = Number(GetUnitprice.unitprice);
                Model.UnitpriceWithVat = Number(GetUnitprice.unitpricewithvat);
                Model.VatPrc = Tax_Rate;//$("#txtTax" + i).val();
    ...
    function Sava_Basket_onclick() {
        if (num_item != 0) {
            ...
            if (!ValidationHeader_On_Chanege())
                return;
            if (ddlCustomer.selectedIndex != 0 && TypeFlag == false) {
                let net = Number($('#All_Total_Basket').attr('All_Total'));
                if (!Check_CreditLimit_Custom(net))
                    return;
            }
