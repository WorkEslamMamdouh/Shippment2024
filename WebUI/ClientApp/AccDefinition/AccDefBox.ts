...
namespace AccDefBox {
    ...
    function Display_Network_account_Code() {
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                if (result.IsSuccess) {
                    //debugger
                    Details_NetworkAcount = result.Response as Array<A_ACCOUNT>;
    ...
    function Display() {
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    Details = result.Response as Array<A_RecPay_D_CashBox>;
    ...
    function DisplayAccDefBox() {
        ...
        for (var i = 0; i < Details.length; i++) {  
            ...
            if (Details[i].IsActive) { $("#checkbox" + i).attr('checked', 'true') }
            else $("#checkbox" + i).removeAttr('checked');

            if (Details[i].IsRecPayAccount) { $("#IsRecPayAccount" + i).attr('checked', 'true') }
            else $("#IsRecPayAccount" + i).removeAttr('checked');
    ...
    function Assign() {
        ...
        for (var i = 0; i < CountGrid; i++) {
            ...
            if (StatusFlag == "i") {
                ...
                if ($("#checkbox" + i).is(':checked')) { Model.IsActive = true; }
                else { Model.IsActive = false; }
                if ($("#IsRecPayAccount" + i).is(':checked')) { Model.IsRecPayAccount = true; }
                else { Model.IsRecPayAccount = false; }
