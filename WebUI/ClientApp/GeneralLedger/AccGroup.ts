...
namespace AccGroup {
    ...
    var SysSession: SystemSession = GetSystemSession(Modules.AccGroup);
    ...
    var LnkTransDetails: Array<A_ACCOUNT_GROUP> = new Array<A_ACCOUNT_GROUP>();
    var _AccGroupMasterDetails: AccGroupMasterDetails = new AccGroupMasterDetails();
    var AccountDetails: A_ACCOUNT = new A_ACCOUNT();
    var AccountDetailsIst: Array<A_ACCOUNT> = new Array<A_ACCOUNT>();

    var txtSearch: HTMLInputElement;
    var txtGROUP_CODE: HTMLInputElement;
    ...
    function InitalizeControls() {
        ...
        txtGROUP_CODE = document.getElementById("txtGROUP_CODE") as HTMLInputElement;
        //print 
    ...
    function GetData_Header() {
        ...
        DataResult(Table);
        //**************************************************************************************************************         
        LnkTransDetails = GetDataTable('A_ACCOUNT_GROUP') as Array<A_ACCOUNT_GROUP>;
    ...
    function _SearchBox_Change() {
        ...
        if (txtSearch.value != "") {
            ...
            let SearchDetails = LnkTransDetails.filter(x => x.GROUP_CODE.toString().search(search) >= 0 || x.DESCA.toLowerCase().search(search) >= 0);
    ...
    function DisplayData(Selecteditem: A_ACCOUNT_GROUP) {
        DocumentActions.RenderFromModel(Selecteditem);
        DisplayDetails(Selecteditem.GROUP_CODE);
    }
    function DisplayDetails(TrID: string) {
        ...
        DataResult(Table);
        //**************************************************************************************************************         
        let List: Array<AQ_GetAccountGroupDetail> = GetDataTable('AQ_GetAccountGroupDetail') as Array<AQ_GetAccountGroupDetail>;
        debugger
        CountGrid = 0;
        $("#div_Data").html(''); 
    ...
    function Chack_CODE(): boolean {
        let Chak_CODE = LnkTransDetails.filter(x => x.GROUP_CODE == $("#txtGROUP_CODE").val().trim())
    ...
    function Assign() {
        ...
        let Master = new A_ACCOUNT_GROUP;
        DocumentActions.AssignToModel(Master);
        Master.COMP_CODE = CompCode;
        Master.GROUP_CODE = $('#txtGROUP_CODE').val(); 


        let Model = new Array<AQ_GetAccountGroupDetail>();
        Model = AssignBuildControls(AQ_GetAccountGroupDetail, CountGrid); 

        debugger
        console.log(Model);
        debugger
         Model.map(name => name.GROUP_CODE.trim() === '' ? name.GROUP_CODE = $('#txtGROUP_CODE').val() : name.GROUP_CODE = name.GROUP_CODE);
    ...
    function Insert() {
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                if (result.IsSuccess) {
                    debugger
                    let res = result.Response as Array<A_ACCOUNT_GROUP>;
    ...
    function Success(ID: string, res:A_ACCOUNT_GROUP) {

        debugger
        IsNew = true;
        btnBack_onclick();

        LnkTransDetails = LnkTransDetails.filter(x => x.GROUP_CODE != ID);
