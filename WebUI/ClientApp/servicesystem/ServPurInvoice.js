...
var ServPurInvoice;
(function (ServPurInvoice) {
    //************system variables
    var SysSession = GetSystemSession(Modules.Ser_Purchasing);
    var compcode;
    var BranchCode;
    var isNew = false;
    var showFlag = false;
    ...
    var btnSave;
    var btnBack;
    var btnAddHeaderControls;
    var btnAddChildControls;
    var btnUpdate;
    var drpSrchStatus;
    var drpImportInvoiceFilter;
    //var chk_ImportInvoice: HTMLInputElement;
    var chkClosed;
    var chk_ImportInvoiceDetail;
    var hd_InvoiceId;
    var txtPerson;
    var txtSearch;
    //------------------------------------- Arrays
    ...
    var ParentHeaderDetails = new Array();
    var PurInvoiceDetail = new Array();
    ...
    var VendorDetail = new A_Pay_D_Vendor();
    ...
    var CountGrid1 = 0;
    var CountGrid2 = 0;
    var GlobalVendorNum = 0; //رقم الفاتورة المختارة 
    ...
    function InitializeControls() {
        ...
        btnAddHeaderControls = document.getElementById("btnAddHeaderControls");
        btnAddChildControls = document.getElementById("btnAddChildControls");
        chkClosed = document.getElementById("chkClosed");
        chk_ImportInvoiceDetail = document.getElementById("chk_ImportInvoiceDetail");
        ...
        txtMenuNum = document.getElementById("txtMenuNum");
        txtRemark = document.getElementById("txtRemark");
        txtPerson = document.getElementById("txtPerson");
        txtSearch = document.getElementById("txtSearch");
        txtInvTotal = document.getElementById("txtInvTotal");
    ...
    function txtSearch_Change() {
        ...
        if (txtSearch.value != "") {
            ...
            searchDetails = AQ_ServPurInvoiceMasterDetailModel.filter(function (x) { return x.TR_NO.toString().search(search_1) >= 0 || x.DocNo.toLowerCase().search(search_1) >= 0
                || x.PERSON.toLowerCase().search(search_1) >= 0; });
    ...
    function FilldrpImportInvoiceFilter() {
        var List_StatusImp = ["نعم", " لا", "الجميع"];
        var List_StatusEnmp = [" yes", " No", "All"];
        if (SysSession.CurrentEnvironment.ScreenLanguage == "ar") {
            for (var i = 0; i < List_StatusImp.length; i++) {
                var newoption = document.createElement("option");
                newoption.value = i.toString();
            ...
            for (var i = 0; i < List_StatusEnmp.length; i++) {
                var newoption = document.createElement("option");
                newoption.value = i.toString();
    ...
    function BindGridData() {
        ...
        Ajax.Callsync({
            ...
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    AQ_ServPurInvoiceMasterDetailModel = result.Response;
    ...
    function DisplayInvoicesDetails(invoiceHeaderID) {
        ModelPurInvoiceDetail = new Array();
        ModelPurInvoiceDetail = HeaderWithDetailModel.AQVAT_GetPurInvoiceDetail.filter(function (s) { return s.InvoiceHeaderID == invoiceHeaderID; });
        CountGrid2 = ModelPurInvoiceDetail.length;
        for (var i = 0; i < CountGrid2; i++) {
            var HeaderObj = DetailList.filter(function (s) { return s.VND_SERIAL == GlobalVendorNum && s.TR_SERIAL == ModelPurInvoiceDetail[i].TR_SERIAL; });
    ...
    function BuildControls_Header(cnt) {
        ...
        $('#btnVendorSrch' + cnt).click(function (e) {
            var sys = new SystemTools();
            sys.FindKey(Modules.Ser_Purchasing, "btnVndorSearch", "CompCode=" + compcode + "and Isactive = 'True' ", function () {
        ...
        $('#btnAccSearch' + cnt).click(function (e) {
            var sys = new SystemTools();
            sys.FindKey(Modules.Ser_Purchasing, "btnAccountSearch", "COMP_CODE=" + compcode + "and ACC_ACTIVE = 1 and DETAIL =1 ", function () {
                var id = SearchGrid.SearchDataGrid.SelectedKey;
                if (GetAccByCode(id)) {
        ...
        $("#chkCash" + cnt).on('change', function () {
            if ($("#txt_StatusFlag1" + cnt).val() != "i")
                $("#txt_StatusFlag1" + cnt).val("u");
            var checked = $("#chkCash" + cnt).prop("checked");
            if (checked == true) {
    ...
    function Validation_Grid_Header(rowcount) {
        if ($("#txt_StatusFlag1" + rowcount).val() == "d" || $("#txt_StatusFlag1" + rowcount).val() == "m") {
            ...
            var checked = $("#chkCash" + rowcount).prop("checked");
    ...
    function BuildControls_Details(cnt) {
        ...
        $('#btnServicSrch' + cnt).click(function (e) {
            var sys = new SystemTools();
            sys.FindKey(Modules.Ser_Purchasing, "btnServiceSearch", "CompCode=" + compcode + "and IsPurchase = 'True' ", function () {
                var id = SearchGrid.SearchDataGrid.SelectedKey;
                $("#txtQuantity" + cnt).val("1");
                $("#txtPrice" + cnt).val("1");
                $("#txtDetailTotal" + cnt).val("0");
        ...
        $('#btnCostCntrSrch' + cnt).click(function (e) {
            var sys = new SystemTools();
            sys.FindKey(Modules.Ser_Purchasing, "btnCostCenterSearch", "COMP_CODE=" + compcode + "and ACTIVE = 1 ", function () {
                var id = SearchGrid.SearchDataGrid.SelectedKey;
    ...
    function Validation_Grid_Detail(rowcount) {
        if ($("#txt_StatusFlag2" + rowcount).val() == "d" || $("#txt_StatusFlag2" + rowcount).val() == "m") {
            ...
            var Qty = Number($("#txtQty" + rowcount).val());
    ...
    function Insert_Serial_Detail(GlobalVendorNum) {
        var Ser = 1;
        for (var i = 0; i < CountGrid2; i++) {
            var flagvalue = $("#txt_StatusFlag2" + i).val();
    ...
    function DeleteRowDetail(RecNo) {
        // 
    ...
    function ValidationDetailBeforeSave(rowcount) {
        if (DetailList[rowcount].StatusFlag == "d" || DetailList[rowcount].StatusFlag == "m") {
            return true;
        }
        else {
            var Qty = Number(DetailList[rowcount].SoldQty);
            var Price = Number(DetailList[rowcount].NetUnitPrice);
    ...
    function AssignDetailsForGrids(GlobalVendorNum, cnt, DetailSerial) {
        var HeaderObj = DetailList.filter(function (s) { return s.VND_SERIAL == GlobalVendorNum && s.TR_SERIAL == DetailSerial; });
        InvDetailAssignSingle = new AVAT_TR_PurInvoiceDetail;
    ...
    function DisplayGridDetails(GlobalVendorNum) {
        $("#div_Data").html("");
        ...
        if (showFlag == true) {
            ...
            DetailArray = DetailList.filter(function (s) { return s.VND_SERIAL == GlobalVendorNum; });
    ...
    function GetVendorByID(id) {
        var VendObj = VendorDetailList.filter(function (s) { return s.VendorID == id; });
    ...
    function btn_arrowdown_onclick(index) {
        ...
        if (CurrentVendorSerial != 0) {
            ...
            DetailList = DetailList.filter(function (x) { return x.VND_SERIAL != CurrentVendorSerial; });
    ...
    function DisplayDetail(DetailForSelectedVendorSR) {
        CountGrid2 = 0;
        var Detailserial = 0;
        $("#div_Data").html("");
        $("#DivChargesShow2").removeClass("display_none");
        for (var i = 0; i < DetailForSelectedVendorSR.length; i++) {
            ...
            var itemobj = ServicesDetails.filter(function (s) { return s.Itemid == DetailForSelectedVendorSR[i].ItemID; });
            if (itemobj.length > 0) {
                $("#txtServCode" + i).val(itemobj[0].ItemCode);
                $("#txtServName" + i).val(itemobj[0].Itm_DescA);
                var catID = itemobj[0].SrvCategoryID;
    ...
    function AssignHeader() {
        var Header = new AVAT_TR_PurInvoiceHeader();
        for (var i = 0; i < CountGrid1; i++) {
    ...
    function AddNewRow_Details() {
        ...
        if (CanAddCharge) {
            ...
            if (CountGrid2 == 0) {
                ...
                var MaxTRSerial = Number($("#txtTR_SERIAL" + (CountGrid2 - 1)).val());
                var NewTRSerial = MaxTRSerial + 1;
            ...
            //Insert_Serial_Detail(GlobalVendorNum);
            Insert_Serial_Detail(GlobalVendorNum);
            var count = DetailList.filter(function (s) { return s.VND_SERIAL == GlobalVendorNum; });
            if (count.length > 0) {
    ...
    function GetMaxVendorSerial() {
        ...
        var MaxVenSrl = yy[yy.length - 1].VND_SERIAL;
        return MaxVenSrl + 1;
    }
    function AddNewRow_Header() {
        ...
        if (CanAddCharge) {
            ...
            if (CountGrid1 == 0) {
                ...
                var MaxVendorSerial = Number($("#VND_SERIAL" + (CountGrid1 - 1)).val());
                var NewVendorSerial = MaxVendorSerial + 1;
...
})(ServPurInvoice || (ServPurInvoice = {}));
//# sourceMappingURL=ServPurInvoice.js.map